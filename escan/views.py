...
#             store=store,
#             order_date__gte=month_ago - timedelta(days=30),
#             order_date__lt=month_ago,
#             status__in=['Completed', 'Delivered']
#         ).aggregate(total=Sum('total_amount'))['total'] or Decimal('0.00')
...
#         # ========== CUSTOMER DATA ==========
#         # Total customers (unique users who purchased from this store)
#         total_customers = CustomerPurchase.objects.filter(store=store).values('customer').distinct().count()
        
#         # New customers this month
#         new_customers_month = CustomerPurchase.objects.filter(
#             store=store,
#             purchase_date__gte=month_ago
#         ).values('customer').distinct().count()
        
#         # Repeat customers
#         repeat_customers = CustomerPurchase.objects.filter(
#             store=store
...
#         completed_orders = Order.objects.filter(store=store, status__in=['Completed', 'Delivered']).count()
...
#         today_customers = CustomerPurchase.objects.filter(
#             store=store,
...
#     except Store.DoesNotExist:
#         # If user doesn't have a store yet
#         context = {
#             'no_store': True
#         }
...
#     try:
#         # Store last action in session instead of global variable
...
#     total_expenses = total_orders.filter(status='Completed').aggregate(
#         total=Sum('total_amount')
#     )['total'] or 0
...
#                 customer_orders = customer_orders.filter(order_date__date=filter_date)
#             except ValueError:
#                 pass
#         elif month:
#             customer_orders = customer_orders.filter(order_date__month=month)
#         elif year:
#             customer_orders = customer_orders.filter(order_date__year=year)
#     else:
#         customer_orders = Order.objects.none()
...
#         # Status positions for visual bars
#         status_top_map = {
#             "Pending": 5,
#             "On Process": 30,
#             "Delivered": 55,
